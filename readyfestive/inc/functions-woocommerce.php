...
if ( ! function_exists( 'storefront_cart_link' ) ) {
	...
	function storefront_cart_link() {
		?>
			<a class="header-cart-contents" href="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'storefront' ); ?>">
            
            <?php if (WC()->cart->get_cart_contents_count() > 0) { ?>
				<span class="count"><?php echo wp_kses_data( sprintf( _n( ' ', WC()->cart->get_cart_contents_count(), 'storefront' ), WC()->cart->get_cart_contents_count() ) );?></span>
               <?php }?>
			</a>
...
if ( ! function_exists( 'storefront_promoted_products' ) ) {
	 ...
	 * @return void
	 */
	function storefront_promoted_products( $per_page = '4', $columns = '4', $recent_fallback = true ) {
		if ( is_woocommerce_activated() ) {
